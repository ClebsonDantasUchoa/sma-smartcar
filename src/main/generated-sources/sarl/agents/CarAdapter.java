...
public class CarAdapter extends Agent {
  ...
  private void $behaviorUnit$UserDetected$2(final UserDetected occurrence) {
    try {
      ...
      InputOutput.<String>println("----ADAPTING CAR----");
      InputOutput.<String>println("CarPreference: ");
      InputOutput.<CarPreference>println(this.carPreferences);
  ...
  @SyntheticMember
  @PerceptGuardEvaluator
  private void $guardEvaluator$UserDetected(final UserDetected occurrence, final Collection<Runnable> ___SARLlocal_runnableCollection) {
    assert occurrence != null;
    assert ___SARLlocal_runnableCollection != null;
    ___SARLlocal_runnableCollection.add(() -> $behaviorUnit$UserDetected$2(occurrence));
  }
